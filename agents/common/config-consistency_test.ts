...
for (const agent of AGENTS) {
  Deno.test(`Registry coverage: ${agent} — directiveType covers all c2 values`, async () => {
    ...
    const allowed = extractPatternValues(dtPattern);
    const c2Values = [
      ...new Set(Object.values(registry.steps).map((s) => s.c2)),
    ];

    const missing = c2Values.filter((v) => !pattern.test(v));
    assert(
      missing.length === 0,
      `Mismatch: .agent/${agent}/steps_registry.json uses c2 values [${
        missing.join(", ")
      }] not in ${CONFIG_DIR}/${agent}-steps-user.yml directiveType pattern [${
        allowed.join(", ")
      }]. ` +
        `IF you added a new step to steps_registry.json, THEN add the c2 value to user.yml directiveType.pattern. ` +
        `IF you intentionally narrowed user.yml, THEN remove the step from steps_registry.json.`,
    );
  });

  Deno.test(`Registry coverage: ${agent} — layerType covers all c3 values`, async () => {
    const registry: RegistryJson = JSON.parse(
      await Deno.readTextFile(`.agent/${agent}/steps_registry.json`),
    );
    const userYml = await Deno.readTextFile(
    ...
    const c3Values = [
      ...new Set(Object.values(registry.steps).map((s) => s.c3)),
    ];

    const missing = c3Values.filter((v) => !pattern.test(v));
    assert(
      missing.length === 0,
      `Mismatch: .agent/${agent}/steps_registry.json uses c3 values [${
        missing.join(", ")
      }] not in ${CONFIG_DIR}/${agent}-steps-user.yml layerType pattern [${
        allowed.join(", ")
      }]. ` +
        `IF you added a new step to steps_registry.json, THEN add the c3 value to user.yml layerType.pattern. ` +
        `IF you intentionally narrowed user.yml, THEN remove the step from steps_registry.json.`,
    );
  });
}
