...
│      ├── verdict            ← When does it stop?            │
│      │   └── type           ← VerdictType (判定戦略)         │
...
Skill が agent 名や `runner.verdict.type`
...
- 詳細: `design/01_philosophy.md`, `design/02_core_architecture.md`
...
1. `entryStepMapping` で `runner.verdict.type` ごとの開始 Step を明示する
...
- `closing` intent は **Completion Loop の起動トリガー** であり、Agent
  完了ではない。 Closure Step が Structured Output で
  `next_action.action: "closing"` を返すと、 Flow は Completion Loop
  に制御を渡す。
- Completion Loop は三段構成（Closure → Validation → Verdict）で Agent
  完了を判定する。 Verdict が `done: false` を返した場合、retryPrompt が Flow
  に戻り作業が継続する。
- `transitions.closing.target` に `null` を明示すると、WorkflowRouter が
  completionSignal と判定し Completion Loop へ制御を渡す。
...
```
Agent 完了シーケンス:
  Closure Step → closing intent → Completion Loop (Closure → Validation → Verdict)
    ├── done=true  → Boundary Hook → Agent 終了
    └── done=false → retryPrompt   → Flow 継続
```

詳細: `design/04_step_flow_design.md` Section 7.1
...
詳細: `design/04_step_flow_design.md`
...
```markdown
Issue #{uv-issue} の作業 プロジェクト: {uv-project_title}
```
...
`runner.verdict.type` に応じて、`entryStepMapping` に必要なキーを定義する。
不足しているとロード時にエラーになる。
...
| [design/07_prompt_system.md](../design/07_prompt_system.md)           | C3L プロンプト解決 |
...
| [design/08_model_selection.md](../design/08_model_selection.md)       | モデル選択設計     |
