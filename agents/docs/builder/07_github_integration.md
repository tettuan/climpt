...
- verdict が closure step を含む
- closing intent が定義されている

#### 実行される操作

1. ラベル更新 (`runner.integrations.github.labels` に基づく)
2. `defaultClosureAction` の値に応じた Issue 操作
...
以下の13パターンが tool-policy でブロックされる:
...
| 10 | `gh release edit`                                                     |
| 11 | `gh api`                                                              |
| 12 | `curl`/`wget`/`python`/`node`/`ruby`/`perl`/`deno` + `api.github.com` |
...
worktree 環境で作業が完了すると、以下の条件を満たす場合に自動で PR が作成される:

- worktree ブランチにコミットが存在する
- リモートに push 済みである
