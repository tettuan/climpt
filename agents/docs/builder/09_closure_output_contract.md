...
## Close binding（どの channel が close を発火するか）

Agent ごとに「どの channel が close を decide するか」を宣言する ADT。
`workflow.json` の `agents.{id}.closeBinding` に書く。詳細・例は
[06_workflow_setup.md](./06_workflow_setup.md) §「Close binding (per agent)」を
権威ソースとして参照する。本節は closure 出力契約から見た縦糸を提示する。

### Structure

```typescript
{
  primary: ClosePrimary;   // 5 variant
  cascade?: boolean;
  condition?: string;      // 旧 closeCondition
}
```

### Mechanism: `primary.kind` と channel routing

`primary.kind` が close を発火する channel を決定する。

| `primary.kind` | Channel ID   | 発火条件                                                           |
| -------------- | ------------ | ------------------------------------------------------------------ |
| `direct`       | 41-channel-D | `TransitionComputed` が terminal phase を決めた時点                |
| `boundary`     | 43-channel-E | `ClosureBoundaryReached` が発火した時点（phase 遷移と独立）        |
| `outboxPre`    | 42-channel-C | `OutboxActionDecided` (filtered to `PreClose`) が発火した時点      |
| `custom`       | 46-channel-U | user-defined `ContractDescriptor.decide` が close を選んだ時点     |
| `none`         | (なし)       | close path 未宣言。Closure step は走るが close action は発火しない |

`closeBinding` 省略は `{ primary: { kind: "none" } }` と等価。`condition` は
dispatch outcome 一致時のみ close を許す guard で、`agents.{id}.outputPhases`
のキーに対応する文字列を書く。
...
### 閉じない agent (handoff-only)

中継 Agent や human-handoff Agent のように **そもそも Issue を close しない**
場合は `kind: "none"` を明示する。これは旧 `closeOnComplete: false`
の置換であり、closure step の structured output で `next_action.action` が
`closing` であっても **close action は発火しない**（Closure 出力契約の 副作用
R1〜R10 のうち、close 系副作用のみが skip される。ラベル / deferred items
は通常通り処理される）。
...
| `closeOnComplete: false`      | `closeBinding.primary = { kind: "none" }` または field 省略 |
| `closeCondition: "<outcome>"` | `closeBinding.condition = "<outcome>"`                      |
...
### R9: `deferred_items` の project 継承（late-binding contract）

`deferred_items` の各 item は optional な `projects` field を持つ
（`13_project_orchestration.md` §2.8）。継承ロジック（Hook O2）:

- `projects` 未指定 + `parentProjects` あり → 親 issue の全 project を継承
- `projects: []` → 継承しない（opt-out）
- `projects: [...]` → 明示指定を使用

継承された project ごとに `add-to-project` outbox action が `create-issue` の
直後に enqueue される。`add-to-project` の `issueNumber` は省略され、
OutboxProcessor の **late-binding contract** により直前の `create-issue`
結果から 解決される（issue #487 Gap 1）。

**不変条件（INV-LATE-BIND）**: OutboxProcessor は `create-issue` の成功結果 （新
issue number）を保持し、後続の `add-to-project` が `issueNumber` 不在の場合
にこの値を注入する。この契約は同一 `process()` 呼び出し内でのみ有効であり、
呼び出し間で state はリセットされる。

### R10: post-close outbox action の順序保証

outbox action に `trigger: "post-close"` を指定すると、通常の `process()` では
スキップされ、saga T6 close 完了後に `processPostClose()` で実行される （issue
#487 Gap 2）。

**用途**: Status=Done field update 等、issue close 後に実行すべき操作。
`trigger` 未指定の action は従来通り close 前（Step 7b）に実行される。

**不変条件（INV-POST-CLOSE）**: `trigger: "post-close"` 付き action は
`closeIssue` より **厳密に後** に実行される。これにより UI 上の不整合 window
（"In progress なのに issue は closed"）を防止する。
