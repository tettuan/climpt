...
- **Agent 完了**: Agent 全体の仕事が終わること。Completion Loop の責務であり、
  外部検証（validationConditions）の結果だけが判定権を持つ。
...
- **共通**: どの VerdictType でも、Flow が Agent 完了を直接判定することはない。
  Flow は Step 遷移だけを担い、Agent 完了は VerdictHandler に委譲する。
...
- Runner は design/07_prompt_system.md
...
| Completion ループ | 実装済 | ValidationChain で Structured Output + 検証条件を統合。        |
