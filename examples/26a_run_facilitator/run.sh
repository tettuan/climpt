...
PROMPT
  ...
  info "Starting facilitator agent without --issue (E2E pipeline test)..."
  show_cmd deno run --allow-all "$REPO_ROOT/agents/scripts/run-agent.ts" --agent facilitator
  local exit_code=0
  output=$(deno run --allow-all "$REPO_ROOT/agents/scripts/run-agent.ts" --agent facilitator 2>&1) \
    || exit_code=$?
