...
main() {
  ...
  if [[ "$verdict_type" != "detect:keyword" ]]; then
    error "FAIL: verdict.type is '${verdict_type}', expected 'detect:keyword'"; return 1
  fi
  success "PASS: verdict.type = detect:keyword"

  # Verify verdictKeyword
  local keyword
  keyword=$(jq -r '.runner.verdict.config.verdictKeyword // empty' "$AGENT_JSON")
  if [[ "$keyword" != "TASK_COMPLETE" ]]; then
    error "FAIL: verdictKeyword is '${keyword}', expected 'TASK_COMPLETE'"; return 1
  fi
  success "PASS: verdictKeyword = TASK_COMPLETE"
  ...
  local initial_prompt="${AGENT_DIR}/prompts/steps/initial/review/f_default.md"
  if [[ ! -f "$initial_prompt" ]]; then
    error "FAIL: initial/review/f_default.md not found"; return 1
  fi
  success "PASS: initial/review/f_default.md exists"

  local closure_prompt="${AGENT_DIR}/prompts/steps/closure/review/f_default.md"
  if [[ ! -f "$closure_prompt" ]]; then
    error "FAIL: closure/review/f_default.md not found"; return 1
  fi
  success "PASS: closure/review/f_default.md exists"
