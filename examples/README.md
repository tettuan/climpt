...
Examples are **numbered 01–44** and designed to run in order. Each step builds
...
21-24   Agent E2E      run, verify, contract, save → plan-mode tested (LLM required)
  ↓
25-34   Agent Contract resolution, run, schema     → agents verified (mixed LLM/local)
        + Run + Verify
  ↓
35-37   Registry       generate, verify, structure → registry.json updated
  ↓
38-39   MCP            server, config              → MCP ready
  ↓
40-43   Workflow       config, resolution,         → workflow CLI E2E verified (--local mode)
                       transition, batch
  ↓
44      Clean          reset all artifacts         → clean state
```
...
> **Note:** Steps 30/34 verify the committed core agents (`.agent/iterator` and
...
- `ANTHROPIC_API_KEY` environment variable — for steps 21-22, 24, 28, 31-32
...
> for f in examples/[0-4][0-9]_*/run.sh; do bash "$f"; done
> ```
...
| ------- | ------------------------------------- | ----------------------------------------- |
| Local   | Deno + jq                             | 01, 03-09, 13-20, 23, 25-27, 29-30, 33-44 |
| Network | `jsr.io` reachable (HTTPS)            | 02, 10-12                                 |
| LLM     | `api.anthropic.com` reachable (HTTPS) | 21-22, 24, 28, 31-32                      |
...
for f in examples/{25,26,27,29,30}_*/run.sh; do bash "$f"; done
for f in examples/{33,34,35,36,37,38,39}_*/run.sh; do bash "$f"; done
for f in examples/{40,41,42,43,44}_*/run.sh; do bash "$f"; done
...
for f in examples/{28,31,32}_*/run.sh; do bash "$f"; done
```

> **Warning: Nested Claude Code execution is not supported.** Examples 21-22,
> 24, 28, 31-32 invoke `deno task agent` which internally spawns a Claude Code
...
| 28 | 28_run_iterator/                  | Run iterator agent (LLM required)                | `.agent/climpt/`     | —                            | contract validation + LLM execution evidence       |
| 29 | 29_negative_agent_load/           | Negative agent load contract                     | —                    | —                            | bad path/JSON/schema → proper errors               |
| 30 | 30_verify_iterator_schema/        | Verify iterator JSON schema (4-level)            | —                    | —                            | file existence, $ref, structure, gate intent       |
| 31 | 31_run_reviewer/                  | Run reviewer agent (LLM required)                | `.agent/climpt/`     | —                            | contract validation + LLM execution evidence       |
| 32 | 32_run_facilitator/               | Run facilitator agent (no --issue)               | `.agent/climpt/`     | —                            | contract validation + LLM execution evidence       |
| 33 | 33_stepkind_tool_policy/          | StepKind tool policy contract                    | —                    | —                            | work/verify deny boundary; closure allows          |
| 34 | 34_verify_reviewer_schema/        | Verify reviewer JSON schema (4-level)            | —                    | —                            | file existence, $ref, structure, gate intent       |
| 35 | 35_generate_registry/             | Generate registry.json                           | `.agent/climpt/`     | `registry.json`              |                                                    |
| 36 | 36_verify_factory_completionpath/ | Verify factory completion path                   | —                    | —                            | completion path coverage verified                  |
| 37 | 37_show_registry_structure/       | Explain registry format                          | `.agent/climpt/`     | —                            |                                                    |
| 38 | 38_mcp_start_server/              | Start MCP server                                 | —                    | MCP running                  | package resolves; server starts without crash      |
| 39 | 39_mcp_show_config/               | MCP integration config guide                     | —                    | —                            |                                                    |
| 40 | 40_workflow_config/               | Workflow config validation (CLI E2E, --local)    | —                    | —                            | valid/invalid config, exit codes, JSON output      |
| 41 | 41_workflow_resolution/           | Label → phase resolution (CLI E2E, --local)      | —                    | —                            | ready/done/blocked/unknown → correct status        |
| 42 | 42_workflow_transition/           | Phase transition via file I/O (CLI E2E, --local) | —                    | —                            | label changes in meta.json after transitions       |
| 43 | 43_workflow_batch/                | Batch processing (CLI E2E, --local)              | —                    | —                            | processed/skipped counts, file system changes      |
| 44 | 44_clean/                         | Cleanup all artifacts                            | all of the above     | —                            |                                                    |
...
for f in examples/[0-4][0-9]_*/run.sh; do
  bash "$f"
done
...
for f in examples/{30,34,36}_*/run.sh; do bash "$f"; done

# Clean up afterwards
bash examples/44_clean/run.sh
```
...
This directory is removed by `44_clean/run.sh`.
