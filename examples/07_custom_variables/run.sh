...
main() {
  ...
  show_cmd "deno run -A $REPO_ROOT/mod.ts run specific --config=test --uv-target=src/init_test.ts"
  ...
  show_cmd "echo \"hello\" | deno run -A $REPO_ROOT/mod.ts echo input --config=test"
  output=$(echo "hello" \
