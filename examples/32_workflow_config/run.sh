...
main() {
  info "=== Workflow Config: Load, Defaults, Validation ==="

  check_deno

  info "Running workflow config tests..."
  show_cmd deno run --allow-read --allow-write --allow-env \
    "${SCRIPT_DIR}/scripts/test-workflow-config.ts"

  output=$(deno run --allow-read --allow-write --allow-env \
    "${SCRIPT_DIR}/scripts/test-workflow-config.ts" 2>&1) \
    || { error "FAIL: workflow config test failed"; return 1; }
  echo "$output"
  ...
  if echo "$output" | grep -q "FAIL:"; then
    error "FAIL: workflow config verification failed"
    fail=1
  fi
