...
const apiGatewayServer = createSdkMcpServer({
  ...
  tools: [
    tool(
      ...
      {
        service: z.enum(["stripe", "github", "openai", "slack"]).describe(
          "Service to call",
        ),
  ...
  tools: [
    tool(
      ...
      "Perform mathematical calculations",
      {
        ...
        precision: z.number().optional().default(2).describe(
          "Decimal precision",
        ),
      },
      async (args) => {
    ...
    tool(
      ...
      "Calculate compound interest for an investment",
      {
        ...
        n: z.number().positive().default(12).describe(
          "Compounding frequency per year",
        ),
      },
      async (args) => {
        const amount = args.principal *
          Math.pow(1 + args.rate / args.n, args.n * args.time);
