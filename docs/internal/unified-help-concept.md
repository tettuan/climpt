...
| `{field}` in file path | 同一イテレーションコンテキストまたは `build.context` のフィールド値で解決 | `.agent/{agentId}/schemas/{outputSchemaRef.file}` |
...
[
  ...
  },
  {
    ...
    "to": {
      "file": ".agent/{agentId}/schemas/{outputSchemaRef.file}",
      "field": "(file)"
    },
    "operator": "exists"
  },
...
| .agent/{agentId}/schemas/{outputSchemaRef.file}       | R-D1~D3 (to)                                                                                                                     |
