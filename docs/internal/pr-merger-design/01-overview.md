...
- **reviewer-agent 出力の verdict 契約**: 既存 reviewer の出力から verdict JSON
  (`.agent/verdicts/<pr-number>.json`) を導出する契約を定義する。reviewer
  プロンプト自体は変更せず、orchestrator の verdict 永続化層で対応する。
...
| **verdict-store**                  | `.agent/verdicts/<pr-number>.json` 配下の verdict 永続化領域。reviewer-agent が書き、merger-cli が読む。                                                                                                                                                       |
...
| ファイル                                     | `.agent/verdicts/<pr-number>.json`                                       | verdict 永続化 (runtime 生成)                                                                                                      |
