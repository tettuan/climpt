...
orchestrator (workflow-merge.json)
    ...
    │                        "--pr", N, "--verdict", ".agent/verdicts/N.json"]) を spawn
    │                       │
...
deno task agent --agent merger --issue <n> --pr <pr> --verdict-path .agent/verdicts/<pr>.json
```
...
```bash
# verdict.json を事前に配置
mkdir -p .agent/verdicts
cat > .agent/verdicts/123.json <<'EOF'
{
...
deno task merge-pr --pr 123 --verdict .agent/verdicts/123.json --dry-run
```
