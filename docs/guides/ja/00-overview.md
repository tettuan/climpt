...
Climpt は5つの層から構成されており、Iterate Agent を頂点とした自律実行を実現しています。
...
**ポイント**: 最上位層と実行層は別々の Claude Agent SDK セッションで動作し、コンテクスト分離により柔軟な自律動作を実現しています。詳細は [05-architecture.md](./05-architecture.md) を参照してください。
