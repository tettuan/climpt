...
| `Claude Code process exited with code 1`       | [3.2 サンドボックス制限](#32-サンドボックス制限)                     |
...
**原因**: `steps_registry.json` の `fallbackKey`
がデフォルトテンプレートキーと一致していない。最も一般的な原因は、アンダースコア区切りではなくドット区切りの形式を使用していること。

**修正**:

1. 失敗しているエージェントの `steps_registry.json` を開く
2. エラーメッセージで参照されているステップを見つける
3. ドット区切りの `fallbackKey` をアンダースコア区切りに変更する:
...
4. 利用可能なキーの完全な一覧は
   [Steps Registry Guide - fallbackKey 命名規則](14-steps-registry-guide.md)
   を参照
...
# 生成された steps_registry.json の fallbackKey と自分のものを比較
```
