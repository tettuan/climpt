...
**permissionMode オーバーライド:**

各ステップは `permissionMode` フィールドを指定することで、エージェントレベルの
SDK パーミッションモードをステップ単位で上書きできます。解決順序:

1. `step.permissionMode` — ステップ単位の明示的オーバーライド
2. `stepKind` デフォルト — `tool-policy.ts` の `STEP_KIND_TOOL_POLICY` から
3. `runner.boundaries.permissionMode` — エージェントレベルの設定

| stepKind       | デフォルト permissionMode | 効果                                 |
| -------------- | ------------------------- | ------------------------------------ |
| `work`         | `acceptEdits`             | 書き込みツール許可                   |
| `verification` | `plan`                    | 読み取り専用（Write/Edit/Bash 拒否） |
| `closure`      | `acceptEdits`             | 書き込みツール許可                   |

stepKind のデフォルトがステップの目的に合わない場合に `permissionMode` を
使用します。例えば、`work` に分類されているが読み取り専用の計画を意図する
architect ステップでは `permissionMode: "plan"` を設定します:
...
利用可能な値: `"plan"`（読み取り専用）、`"acceptEdits"`（書き込み許可）、
`"bypassPermissions"`（プロンプトなし — `--dangerously-skip-permissions`
が必要）。
...
- `uvVariables` で参照される UV 変数が実行時に非空の値を持つことを確認するか、
  ステップ定義から省略すること
