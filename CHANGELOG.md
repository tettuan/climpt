...
### Changed
- **Breaking**: agent.json config restructured from flat `behavior`/`prompts`/`logging`/`github`/`worktree` to `runner.*` hierarchy (`runner.flow`, `runner.completion`, `runner.boundaries`, `runner.execution`, `runner.telemetry`). See [migration guide](agents/docs/builder/migration_guide.md#v1120-config-migration-behavior---runner) for field mapping.
