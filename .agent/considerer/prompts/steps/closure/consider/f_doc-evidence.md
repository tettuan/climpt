...
- `uv-doc_paths_required` — list from `closure.consider.doc-scan`.
...
- `doc_evidence: [{path, diffed, commits: [{sha, date, subject, stat}], truncated}]`
  one entry per path in `doc_paths_required`, in input order.
...
4. Emit intent `next` (proceed to `closure.consider.precheck-kind-read`).

## Output

Return a JSON object matching `closure.consider.doc-evidence` in
...
- Collect metadata into `doc_evidence` and emit `next`.

## Do NOT
- Read the diff body (`git show` without `--stat`, `git diff`, etc.). Hunk-level
  content is intentionally out of scope here — the `consider` terminal step has
  full read access if it needs to drill in.
- Judge whether any commit "resolves" the issue. Relevance is for `consider` to
  decide.
- Read the issue body, comments, or any source file.
- Post issue comments or modify any state.
- Emit intents other than `next` or `repeat` (on git/gh failure).
- Exceed `--max-count=20`. The cap is intentional to bound cost on long-lived
  issues; `truncated: true` signals the cap was hit.

## Error handling
- `gh issue view` failure → emit `repeat` with `next_action.reason` describing
  the gh error.
- `git log` / `git show` failure on a specific path → emit `repeat` with
  `next_action.reason` naming the failing path. Do not fall back to partial
  output; the runner will retry.
