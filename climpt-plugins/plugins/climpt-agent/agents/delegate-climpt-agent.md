...
This agent connects Claude Code to Climpt by spawning independent sub-agents using Claude Agent SDK. When a user's request matches a Climpt command, this agent creates a short query text that describes the user's intent, then executes the `climpt-agent.ts` script. The script internally handles the multi-stage workflow (search → describe → execute) and runs an isolated sub-agent to complete the task.
