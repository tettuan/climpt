---
name: logs-analysis
description: Use when investigating tmp/logs contents — orchestrator session JSONL, per-agent JSONL, or examples stdout/stderr. Uses deterministic TS digest scripts. Haiku sub agent is used ONLY for file location (running index-logs.ts); all interpretation stays in main. Trigger phrases — "tmp/logs を調べる", "session log を見て", "なぜ orchestrator が止まった", "agent のイテレーション", "examples の失敗ログ", "analyze logs", "ログ調査".
...
| 解釈 (分類 / success/fail 判定 / RCA / 次アクション) | **main (Opus)** | digest JSON を読んで判断 |
...
- Haiku に index-logs.ts 以外の script 実行権限を広く渡さない (必要なら main が直接回す)
...
### Step 2. What — digest 生成 (main が直接実行)
...
### Step 3. Interpret — main (Opus) が解釈

digest JSON を main が読んで判断する:
...
4. main → 2 つの digest を main 自身で突き合わせ、`maxIterationHits` や `dispatchOutcomes.fail` の起点を読む
5. main → RCA と次アクションを main が書く (sub agent に書かせない)
```
