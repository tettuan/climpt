...
Deno.test("Registry template has correct structure", async () => {
  const templatePath = "examples/48_mcp_show_config/registry.template.json";
  ...
  try {
    ...
    assertExists(
      template.tools.availableConfigs,
      "Template should have availableConfigs",
    );
    ...
    for (const command of template.tools.commands) {
      ...
      assertExists(
        command.description,
        "Template command should have description",
      );
    ...
    if (registry.tools?.commands) {
      for (const command of registry.tools.commands) {
        if (command.options) {
          ...
          // Check file, stdin, destination are booleans
          assertEquals(
